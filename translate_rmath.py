"""
RMath 데이터셋 번역 파이프라인 (translate_rmath.py)

- 'utils.py'의 공통 파이프라인 실행기를 사용합니다.
- RMath 고유의 로직 (수학/LaTeX 보존 프롬프트)을 정의하여 주입합니다.
- RTool과 달리 tools, tool_calls, metadata 로직이 없습니다.
"""
...
# ==============================================================================
# RMath Specific Configuration
# ==============================================================================
# RMath 데이터셋 경로 설정
utils.DATASET_PATH = "/data/ib-a100-cluster-a-pri-lmalign_942/personal/ryan/project/data-api/sample/rmath-0528"
DATASET_PATH = utils.DATASET_PATH # main_runner에 전달하기 위함

# ==============================================================================
# RMath SPECIFIC FUNCTIONS
# ==============================================================================

# 1. RMath용 프롬프트 (수학/LaTeX 보존)
# ==============================================================================

def create_rmath_translation_prompt(text: str) -> str:
    """
    (RMath 엄격) 번역 프롬프트: 'content' 및 'reasoning_content'용.
    LaTeX, 수식, 변수, 코드 등을 엄격하게 보존합니다.
    """
...
# ==============================================================================
# 2. RMath용 배치 입력 생성 (utils.run_batch_pipeline에 주입될 함수)
# ==============================================================================

def prepare_rmath_batch_input(
    dataset: Dataset,
    model: str,
    ...
    """
    (RMath Specific) Batch API 입력을 위한 *요청 리스트*를 생성합니다.
    (파일 분할 및 저장은 utils.py에서 처리)

    - RMath는 'metadata'나 'tools'를 사용하지 않습니다.
    - 'user'와 'assistant' 역할만 존재합니다.
    - 'content'와 'reasoning_content' 모두 동일한 엄격한 수학용 프롬프트를 사용합니다.
    """
        ...
        for msg_idx, message in enumerate(messages):
            ...
            if reasoning_content and reasoning_content.strip():
                ...
                for chunk_idx, chunk in enumerate(reasoning_chunks):
                    ...
                    # RMath는 reasoning도 항상 엄격한 수학용 프롬프트 사용
                    prompt_content = create_rmath_translation_prompt(chunk)
...
# ==============================================================================
# 3. RMath용 실패 레코드 추출 (utils.handle_retry_failures에 주입될 함수)
# ==============================================================================

def extract_rmath_failed_records(dataset: Dataset, failed_ids: List[str]) -> Dict[str, Dict[str, Any]]:
    """
    (RMath Specific) Extract records and messages that correspond to failed custom IDs.
    - 재시도에 필요한 'content' 본문만 추출합니다. (role, tool_calls 등 불필요)
    """
    ...
    for custom_id in failed_ids:
        ...
        try:
            ...
            if record_idx < len(dataset):
                ...
                # RMath는 metadata/tools_json이 필요 없음
...
# ==============================================================================
# 4. RMath용 재시도 요청 생성 (utils.handle_retry_failures에 주입될 함수)
# ==============================================================================

def prepare_rmath_retry_requests(
...
if __name__ == "__main__":
    # utils.main_runner에 RMath 고유의 설정과 함수들을 주입합니다.
    asyncio.run(utils.main_runner(
        dataset_path=DATASET_PATH,
        prepare_batch_input_fn=prepare_rmath_batch_input,
        extract_failed_records_fn=extract_rmath_failed_records,
        prepare_retry_requests_fn=prepare_rmath_retry_requests
    ))
